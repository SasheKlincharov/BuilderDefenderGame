...
public class ResourceGenerator : MonoBehaviour {
    ...
    public static int GetNearbyResourceAmount(ResourceGeneratorData resourceGeneratorData, Vector3 position) {
        ...
        foreach (Collider2D collider2D in collider2DArray) {
            ...
            if (resourceNode != null) {
                // This obj is indeed a resource node
                if (resourceNode.resourceType == resourceGeneratorData.resourceType) {
                    // Same type
                    nearbyResourceAmount++;
                } else {
                    // Display a tooltip indicating wrong resource node!
                }
            }
        }
    ...
    private void Start() {
        ...
        if (nearbyResourceAmount == 0) {
            // No nearby resource nodes..
            // Disable resource generator
            enabled = false;
        } else {
            // We start at timerMax half and then we add the timerMax based on how many nodes we found
